...
function createLogger (options) {
  ...
  const { createPinoLogger, createSerializers } = require('./logger-pino.js')
  ...
  if (validateLogger(options.loggerInstance)) {
    const logger = createPinoLogger({
      ...
      serializers: Object.assign({}, createSerializers(options.loggerInstance.customAttributeKeys || {}), options.loggerInstance.serializers)
    })
  ...
  localLoggerOptions.serializers = Object.assign({}, createSerializers(localLoggerOptions.customAttributeKeys || {}), localLoggerOptions.serializers)
  options.logger = localLoggerOptions
  const logger = createPinoLogger(options.logger)
  return { logger, hasLogger: true }
}

function getSerializerKeys (logger) {
  const symbols = Object.getOwnPropertySymbols(logger)

  // Find the symbol for serializers
  const serializersSymbol = symbols.find(sym => sym.toString() === 'Symbol(pino.serializers)')

  if (serializersSymbol) {
    // Access the serializers if the symbol was found
    const serializers = logger[serializersSymbol]

    // Access individual serializers
