...
const { createChildLogger, getSerializerKeys } = require('./logger-factory.js')
...
function buildRouting (options) {
  ...
  function routeHandler (req, res, params, context, query) {
    ...
    const [, reqKey = 'req', resKey = 'res'] = getSerializerKeys(logger)
    ...
    if (closing === true) {
      ...
      if (return503OnClosing) {
        ...
        childLogger.info({ [resKey]: { statusCode: 503 } }, 'request aborted - refusing to accept new requests as server is closing')
        return
      }
    }
